            ...
            if selected_to_date:
                ...
                for index, row in events.iterrows():
                    if row['event_id'] not in event_options.keys():
                        starts_converted_to_timezone = pytz.timezone('Europe/Vienna').localize(row['starts']).astimezone(pytz.timezone(st.session_state.timezone)).replace(tzinfo=None).replace(second=0)
