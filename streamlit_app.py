                ...
                if selected_bet_status != '()':
                    ...
                    if user_unique_starts is not None:
                        ...
                        if 'initial_df' not in st.session_state:
                            st.session_state['initial_df'] = placeholder1.data_editor(styled_df, column_config={"DEL": st.column_config.CheckboxColumn("DEL", help="Select if you want to delete this bet.", default=False), "ST": st.column_config.TextColumn("ST", help="Bet Status. W = Won, HW = Half Won, L = Lost, HL = Half Lost, P = Push, V = Void, na = ungraded. This will be settled automatically. Please be patient, this can take up to several hours. On rare occasions it is possible that the payout or grading you received from your sportsbook is different (i.e. due to settlement rules if a match is abandoned or due to a retirement of a player). You can edit the value by doublicking on the cell."), "TAG": st.column_config.Column("TAG", help="Tag your bets to classify them for future research, i.e. apply a tag filter. This could be a particular strategy, model or a tipster, etc. You can edit the value with a doublick on the cell."), "SH": st.column_config.Column("SH", help="Score Home. This will be settled automatically. Please be patient, this can take up to several hours. On rare occasions it is possible that the payout or grading you received from your sportsbook is different (i.e. due to settlement rules if a match is abandoned or due to a retirement of a player). You can edit the value by doublicking on the cell."), "SA": st.column_config.Column("SA", help="Score Away. This will be settled automatically. Please be patient, this can take up to several hours. On rare occasions it is possible that the payout or grading you received from your sportsbook is different (i.e. due to settlement rules if a match is abandoned or due to a retirement of a player). You can edit the value by doublicking on the cell."), "STARTS": st.column_config.Column("STARTS", help="Event starting time"), "SPORT": st.column_config.Column("SPORT", help="Sport"), "LEAGUE": st.column_config.Column("LEAGUE", help="League"), "RUNNER_HOME": st.column_config.Column("RUNNER_HOME", help="Home Team/Player 1"), "RUNNER_AWAY": st.column_config.Column("RUNNER_AWAY", help="Away Team/Player 2"), "MARKET": st.column_config.Column("MARKET", help="Market. This can be one of the following: MONEYLINE, SPREAD, TOTALS, HOME_TOTALS, AWAY_TOTALS"), "PERIOD": st.column_config.Column("PERIOD", help="Period. This refers to the game section of the bet, i.e. fulltime, halftime, 1st quarter, etc."), "SIDE": st.column_config.Column("SIDE", help="Selection"), "LINE": st.column_config.Column("LINE", help="Line refers to the handicap for spread & totals markets."), "ODDS": st.column_config.Column("ODDS", help="Obtained price"), "STAKE": st.column_config.Column("STAKE", help="Risk amount"), "BOOK": st.column_config.Column("BOOK", help="Bookmaker. You can edit the value with a doublick on the cell."), "Your P/L will be settled automatically. Please be patient, this can take up to several hours. On rare occasions it is possible that the payout or grading you received from your sportsbook is different (i.e. due to settlement rules if a match is abandoned or due to a retirement of a player). You can edit the value by doublicking on the cell.": st.column_config.Column("P/L", help="Actual profit"), "CLS": st.column_config.Column("CLS", help="Closing price"), "CLS_TRUE": st.column_config.Column("CLS_TRUE", help="Closing price with bookmaker margin removed (= no-vig closing odds)"), "CLS_LIMIT": st.column_config.Column("CLS_LIMIT", help="Maximum bet size at closing"), "EXP_WIN": st.column_config.Column("EXP_WIN", help="Expected Win. This is the expected value of your bet. This figure compares obtained odds with no-vig closing odds and takes into account the stake. Quality bets will typically have an exp_win > 0."), "CLV": st.column_config.Column("CLV", help="Closing line value. This is the expected roi of your bet. This figure compares obtained odds with no-vig closing odds. Quality bets will typically have a clv > 0."), "BET_ADDED": st.column_config.Column("BET_ADDED", help="Timestamp of the recorded bet.")}, disabled=['STARTS', 'SPORT', 'LEAGUE', 'RUNNER_HOME', 'RUNNER_AWAY', 'MARKET', 'PERIOD', 'SIDE', 'LINE', 'ODDS', 'CLS', 'CLS_TRUE', 'CLS_LIMIT', 'EXP_WIN', 'CLV', 'BET_ADDED', 'ID', 'STAKE'], key='initial_df_key', hide_index=True)
                            placeholder1.empty()
